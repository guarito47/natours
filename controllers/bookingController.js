...
exports.getCheckoutSession = catchAsync(async (req, res, next) => {
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    success_url: `${req.protocol}://${req.get('host')}/`, // we redirect home after success
    ...
    line_items: [//this is an array of items to pay, in this case just one tour
      {
        ...
        /*name: `${tour.name} Tour`, // we send the tour name to stripe
        description: tour.summary, // we send the tour summary to stripe
        // this is an array of images, we send the tour cover image to stripe
        //images: [`${req.protocol}://${req.get('host')}/img/tours/${tour.imageCover}`],
        //for testing we are using a static image, because is mandatory have a image on the web
        images: ['https://www.natours.com/img/tours/tour-1-cover.jpg'], // we can use a static image for testing         
        amount: tour.price * 100, // we send the tour price to stripe in cents
        currency: 'usd', // we set the currency to usd
        quantity: 1 // we set the quantity to 1*/
